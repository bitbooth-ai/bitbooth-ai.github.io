...
## Fifth batch (2026-07-11) — image-led animation round, all three sketches approved
Brief: almost zero text, just images in very nice animation, using Motion or three.js. Three concepts (A cake stand / B flying gallery / C sugar drift) were pitched; the owner approved all three. Copy in each is reduced to a wordmark, a one-line hint, and a single "call me back" button opening a minimal `<dialog>` form (email/phone required). All keep the floor criteria (light, joyful, mobile) and honor prefers-reduced-motion (auto-rotation/bobbing off, camera/springs become instant). Both three.js designs show a graceful fallback message if WebGL/CDN is unavailable.

- **design-31 — The Cake Stand (three.js r128, cdnjs).** A 3D cake stand in cream fog: gold column and finial, three pastel gilt-rimmed plates (pink = macarons, butter = Thai sweets, mint = flowers) each carrying a rotating ring of product photos. Scroll flies the camera down past the spinning tiers; horizontal drag spins the nearest tier with inertia; tapping a photo detaches it from the ring and floats it up to the camera (tap again to send it home). CTA fades in at the bottom of the descent. Fraunces.
- **design-32 — The Flying Gallery (three.js r128, cdnjs).** A slow scroll-driven flight through blush fog: twelve white-framed photos hang in 3D space, alternating sides, gently bobbing and tilting; 26 pastel candy orbs drift for atmosphere; cursor position parallaxes the camera. Fog swallows and reveals each frame. Ends floating at "every piece, by one pair of hands" + the button. Cormorant Garamond. *Gallery-hush counterpart to 31's playfulness.*
- **design-33 — Sugar Drift (Motion One 10.18, jsdelivr; WAAPI fallback if the CDN fails).** No WebGL: ten polaroid-style photo cards scattered across a fixed stage, each bobbing on its own rhythm. Scrolling brings cards one at a time to center with a spring bounce (stiffness 110/damping 15); any card can be grabbed and tossed and springs back to where it belongs. The lightest of the three — smoothest on cheap phones. Quicksand.
